...
jStat.extend( jStat.fn, {
	...
	max: function() { 
	  var row_cnt = 0, col_cnt = 0, value,
	  row_max = this.rows(),
	  col_max = this.cols(),
	  max = null;
	  for ( row_cnt = 0 ; row_cnt < row_max; row_cnt++) {
	    for ( col_cnt = 0; col_cnt < col_max; col_cnt++) {
		value = this[row_cnt][col_cnt];
...
jStat.extend({
	...
	// Calculate the square of the value being passed
	sq: function(value) {
		return value*value;
	},

	// Multiply a vector with a scalar and returns a vector ( not a matrix)
	vec_mult: function(arr, factor) {
		var i,res = [];
